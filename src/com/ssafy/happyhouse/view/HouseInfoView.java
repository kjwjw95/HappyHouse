...
public class HouseInfoView{
   ...
   private void showHouseInfo(int code) {
     ...
     // System.out.println(curHouse);
      ...
      houseInfoL[10].setText("");
      houseInfoL[11].setText(Integer.toString(curHouse.getPoison()));
      houseInfoL[12].setText(Integer.toString(curHouse.getAir()));
      houseInfoL[13].setText(Integer.toString(curHouse.getWater()));
...
//      });
   ...
   public void setMain(){
      ...
      JPanel leftCenter = new JPanel(new GridLayout(1, 1));
      JPanel leftR = new JPanel(new GridLayout(15, 2));
      leftR.setBorder(BorderFactory.createEmptyBorder(0 , 10 , 10 , 10));
      String[] info= {"","","주택명","거래금액","월세금액","건축연도","전용면적","거래일","법정동","지번","      주변 환경정보","유독물","대기오염","수질오염"};
      ...
      imgL = new JLabel();
      commercialBt= new JButton(" 상권 표시"); 
      leftR.add(commercialBt);
      
      commercialBt.addActionListener(e ->{
          new newWindow(curHouse);
       });
...
class newWindow extends JFrame {
	...
	private DefaultTableModel    houseModel;
	private String[]           titles = { "번호", "대번호", "중분류", "소분류", "상호명"};
	private HouseService houseService;
	private HouseDeal cur;
	newWindow(HouseDeal curHouse) {
		setTitle(curHouse.getDong() + " 상권");
		cur=curHouse;
		JFrame frame;
		houseService = new HouseServiceImpl();
		frame = new JFrame("HappyHouse -- 상권 정보");
		JPanel NewWindowContainer = new JPanel();
		setContentPane(NewWindowContainer);

//		JLabel NewLabel = new JLabel(curHouse.getDong());
//
...
//		setVisible(false);
		...
		JPanel rightTop1 = new JPanel(new GridLayout(1, 4));
//		String[] chooseMeg = { "아파트 매매", "아파트 전월세", "주택 매매", "주택 전월세" };
//		JCheckBox[] chooseC = new JCheckBox[chooseMeg.length];
//		for (int i = 0, len = chooseMeg.length; i < len; i++) {
//			chooseC[i] = new JCheckBox(chooseMeg[i], true);
//			rightTop1.add(chooseC[i]);
//		}
		rightTop1.add(new JLabel(curHouse.getDong()));
		JPanel rightTop2 = new JPanel(new GridLayout(1, 3));
	      String[] item = {"---all---","대분류","중분류","소분류"}; 
	      JComboBox findC = new JComboBox<String>(item);
	      ...
	      JPanel rightCenter = new JPanel(new BorderLayout());
	      houseModel = new DefaultTableModel(titles,20);
	      JTable houseTable = new JTable(houseModel);
	      JScrollPane housePan = new JScrollPane(houseTable);
	      houseTable.setColumnSelectionAllowed(true);
	      rightCenter.add(new JLabel("거래 내역", JLabel.CENTER),"North");
	      rightCenter.add(housePan,"Center");
	      ...
	      List<CS> deals = houseService.searchCS(cur.getDong());
	      if(deals!=null){
	         int i=0;
	         String[][]data = new String[deals.size()][5];
	         for (CS deal: deals) {
	            data[i][0]= deal.getNum();
	            data[i][1]= deal.getCode1();
	            data[i][2]= deal.getCode2();
	            data[i][3]= deal.getCode3();
	            data[i++][4]= deal.getName();
	         }
	         houseModel.setDataVector(data, titles);
	      }
